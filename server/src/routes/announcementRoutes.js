...
export function registerAnnouncementRoutes(app, { io, upload, saveUploadedImage }) {
  app.get('/api/admin/announcements', adminAuth, async (req, res) => {
    try {
      res.json(await all('SELECT * FROM announcements ORDER BY sort_order ASC, created_at DESC'));
    } catch (err) {
  ...
  app.post('/api/admin/announcements', adminAuth, async (req, res) => {
    try {
      ...
      await run('INSERT INTO announcements (id, title, message, type, is_active, start_date, end_date, sort_order, image_url, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
  ...
  app.patch('/api/admin/announcements/:id', adminAuth, async (req, res) => {
    try {
      ...
      console.error('PATCH /api/admin/announcements/:id failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
  ...
  app.delete('/api/admin/announcements/:id', adminAuth, async (req, res) => {
    try {
      ...
      console.error('DELETE /api/admin/announcements/:id failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
}
