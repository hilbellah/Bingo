...
export function registerAdminSessionRoutes(app, { io, logAudit }) {
  app.get('/api/admin/sessions', adminAuth, async (req, res) => {
    try {
      ...
      res.json(await all(`SELECT * FROM sessions ${where} ORDER BY date ASC, time ASC`));
    } catch (err) {
    ...
    try {
      ...
      for (let tableNumber = 1; tableNumber <= 75; tableNumber++) {
        for (let chairNumber = 1; chairNumber <= 6; chairNumber++) {
          await run('INSERT INTO seats (id, session_id, table_number, chair_number, status) VALUES (?, ?, ?, ?, ?)',
  ...
  app.patch('/api/admin/sessions/:id', adminAuth, async (req, res) => {
    try {
      ...
      if (req.body.doors_open_time !== undefined) {
        const doorsOpen = effectiveSessionType === 'event' || effectiveSessionType === 'special_bingo'
          ? normalizeDoorsOpenTime(req.body.doors_open_time)
          : { value: null };
        if (doorsOpen.error) return res.status(400).json({ error: doorsOpen.error });
        updates.push('doors_open_time = ?');
        values.push(doorsOpen.value);
      ...
      await run(`UPDATE sessions SET ${updates.join(', ')} WHERE id = ?`, values);

      let notifiedBookings = 0;
      if (shouldNotifyReschedule) {
        const updatedSession = await get('SELECT * FROM sessions WHERE id = ?', [req.params.id]);
        const paidBookings = await all(`
        ...
        await logAudit('session_rescheduled', 'session', req.params.id, {
  ...
  app.delete('/api/admin/sessions/:id', adminAuth, async (req, res) => {
    try {
      ...
      console.error('DELETE /api/admin/sessions/:id failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
  ...
  app.get('/api/admin/packages', adminAuth, async (req, res) => {
    try {
      res.json(await all('SELECT * FROM packages ORDER BY sort_order ASC'));
    } catch (err) {
  ...
  app.post('/api/admin/packages', adminAuth, async (req, res) => {
    try {
      ...
      const id = uuid();
      await run('INSERT INTO packages (id, name, price, type, max_quantity, is_active, sort_order, is_phd, description) VALUES (?, ?, ?, ?, ?, 1, ?, ?, ?)',
  ...
  app.patch('/api/admin/packages/:id', adminAuth, async (req, res) => {
    try {
      ...
      if (updates.length === 0) return res.status(400).json({ error: 'No fields' });
      values.push(req.params.id);
      await run(`UPDATE packages SET ${updates.join(', ')} WHERE id = ?`, values);
      res.json({ success: true });
    } catch (err) {
  ...
  app.delete('/api/admin/packages/:id', adminAuth, async (req, res) => {
    try {
      ...
      const pkg = await get('SELECT id, name FROM packages WHERE id = ?', [id]);
      if (!pkg) return res.status(404).json({ error: 'Package not found' });

      const itemRow = await get('SELECT COUNT(*) AS c FROM booking_items WHERE package_id = ?', [id]);
      const addonRow = await get('SELECT COUNT(*) AS c FROM booking_addons WHERE package_id = ?', [id]);
  ...
  app.patch('/api/admin/seats/:id', adminAuth, async (req, res) => {
    try {
      ...
      console.error('PATCH /api/admin/seats/:id failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  app.patch('/api/admin/sessions/:id/tables/:tableNumber/seats', adminAuth, async (req, res) => {
    try {
      const { is_disabled } = req.body;
      ...
      const session = await get('SELECT id, date, time, event_title FROM sessions WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
      if (!session) return res.status(404).json({ error: 'Session not found' });
      ...
      for (const seat of editableSeats) {
        await run('UPDATE seats SET is_disabled = ? WHERE id = ?', [targetValue, seat.id]);
      }

      await logAudit(is_disabled ? 'table_disabled' : 'table_enabled', 'session', session.id, {
      ...
      console.error('PATCH /api/admin/sessions/:id/tables/:tableNumber/seats failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  app.patch('/api/admin/sessions/:id/seats/bulk', adminAuth, async (req, res) => {
    try {
      ...
      await logAudit(is_disabled ? 'bulk_seats_disabled' : 'bulk_seats_enabled', 'session', session.id, {
        changed_seats: editableSeats.length,
        skipped_sold_seats: seats.length - editableSeats.length,
      ...
      console.error('PATCH /api/admin/sessions/:id/seats/bulk failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  app.get('/api/admin/sessions/:id/seats', adminAuth, async (req, res) => {
    try {
      await releaseExpiredHolds(io);
      const seats = await all(`
  ...
  app.get('/api/admin/sessions/deleted', adminAuth, async (req, res) => {
    try {
      await archivePastSessions();
      ...
      console.error('GET /api/admin/sessions/deleted failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  app.get('/api/admin/sessions/:id/bookings', adminAuth, async (req, res) => {
    try {
      ...
      console.error('GET /api/admin/sessions/:id/bookings failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  app.post('/api/admin/sessions/:id/restore', adminAuth, async (req, res) => {
    try {
      const session = await get('SELECT * FROM sessions WHERE id = ? AND deleted_at IS NOT NULL', [req.params.id]);
      if (!session) return res.status(404).json({ error: 'Deleted session not found' });

      await run('UPDATE sessions SET deleted_at = NULL WHERE id = ?', [req.params.id]);
      await logAudit('session_restored', 'session', req.params.id, { date: session.date, time: session.time });
      await saveDb();
      res.json({ success: true });
    } catch (err) {
      console.error('POST /api/admin/sessions/:id/restore failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });
  ...
  app.get('/api/admin/audit-log', adminAuth, async (req, res) => {
    try {
      ...
      const logs = await all(`SELECT * FROM audit_log ${whereClause} ORDER BY created_at DESC LIMIT ?`, [...params, limitVal]);
  ...
  app.get('/api/admin/sessions/:id/packages', adminAuth, async (req, res) => {
    try {
      const session = await get('SELECT is_special_event, session_type FROM sessions WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
      if (!session) return res.status(404).json({ error: 'Session not found' });
      ...
      res.json(await all('SELECT * FROM session_packages WHERE session_id = ? ORDER BY sort_order ASC', [req.params.id]));
    } catch (err) {
      console.error('GET /api/admin/sessions/:id/packages failed:', err);
      res.status(500).json({ error: 'Internal server error' });
    }
  });

  app.post('/api/admin/sessions/:id/packages', adminAuth, async (req, res) => {
    try {
