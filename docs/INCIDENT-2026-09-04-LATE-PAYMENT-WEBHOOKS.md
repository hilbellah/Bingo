...
| 10 | **Degraded-gateway seat protection.** Two consecutive failures to reach the gateway put the reconciler in degraded mode: every in-flight checkout's seats are held for 60 min (released ones re-held), re-applied each cycle, audit-logged, shown on `/health/payments`. Seats never go back on sale while payments cannot be seen. | `checkoutGuards.js`, `paymentReconciliation.js` |
| 11 | **30-minute hold once at the card form** (browsing holds stay 20). | `checkoutRoutes.js`, `CHECKOUT_HOLD_MINUTES` |
| 12 | **Payment after cancel or after a decline** claims the seat if it is still free (was: quarantined even with the seat free). Logged as `payment_after_<status>_reclaimed`. | `bookingPayments.js` |
| 13 | **Admin guards:** disabling a seat/table, assigning a promo ticket, moving a ticket onto a seat, or deleting a session is refused (409 `checkout_in_progress`) while a customer is at the card form for that seat; super users may `?force=1`, audit-logged. | `checkoutGuards.js`, `adminSessionRoutes.js`, `adminBookingRoutes.js` |
| 14 | **Longer memory:** reconciler and audit look back 7 days; stale cleanup leaves card-form checkouts for 7 days. | `paymentReconciliation.js`, `paymentAudit.js`, `scheduler.js` |
| 15 | **Escalation:** a review open > 2 h turns `/health/payments` red (503) and the 6-hourly audit re-emails it as a REMINDER; dismissed reviews do not count. | `index.js`, `paymentAudit.js`, `email.js` |
